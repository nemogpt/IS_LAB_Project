...
def KDF(sharedKey, count=1000):
    derivedKey = PBKDF2(str(sharedKey), b"",
                        dkLen=CIPHER_KEY_SIZE, count=count)
    return derivedKey
        ...
        for toId in self.publicKeys:
            if toId not in self.secretKeys:
                if (toId == self.id):
                    ...
                    self.secretKeys[toId] = pow(
                        self.publicKeys[toId], self.privateKey, self.p)
...
class Server(object):
    ...
    def addUser(self, id):
        ...
        for i in self.idList:
            # When user is online should check updated users and update
            self.users[i].updateIdList()
            if i != id:
